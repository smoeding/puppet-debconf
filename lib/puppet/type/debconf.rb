# debconf.rb --- Type for the debconf provider
...
Puppet::Type.newtype(:debconf) do
  ...
  newparam(:item, :namevar => true) do
    desc %{The item name. This string must have the following format:
      the package name, a literal slash char and the name of the question.}

    newvalues(/^[a-z0-9][a-z0-9:.+-]+\/[a-zA-Z0-9\/_.+-]+$/)
  end

  newparam(:package) do
    desc %{The package the item belongs to. The default is the first part
      (up to the first '/') of the item parameter.}
  ...
  newparam(:type) do
    desc %{The type of the item. This can be one of the following values:
      string, boolean, select, multiselect, note, text, password, title}
